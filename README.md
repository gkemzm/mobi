...
또한 데이터가 저장될 atom 폴더와 server 호출 api 함수를 lib 폴더에 같이 묶어 개발자의 상호 접근성을 높였습니다.

## 컴포넌트 설계

컴포넌트 관련으로는 모달 컴포넌트 쪽이 있는것 같습니다.
기능성 테이블 리스트도 가능하지만 일관된 데이터 스키마에 따라 변동이 크지만
모달은 미리 레이어를 따놓고 children을 받을 수 있어 해당부분은 재사용성을 고려했습니다.

번외로 api통신쪽은 customhook을 활용해 어디서든 편하게 호출 수 있도록 하였습니다.
